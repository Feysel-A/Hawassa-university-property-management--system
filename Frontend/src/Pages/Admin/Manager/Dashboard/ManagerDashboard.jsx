...
const ManagerDashboard = () => {
  ...
  const handleAllocate = (request) => {
    ...
    setMessage(`Allocated ${request.assetName} successfully!`);
    setTimeout(() => setMessage(""), 3000);
  };

  const handleReject = (request) => {
    const rejectionData = { ...request, status: "Rejected", rejectedBy: "MGR001" };
          ...
          <CRow>
            <CCol xs={12}>
              <p className="text-muted">
                Welcome, [Property Manager]! Manage asset allocations here.
              </p>
            </CCol>
          </CRow>
          {message && (
            <CAlert
              color={message.includes("Allocated") ? "success" : "danger"}
              className="mb-4"
            >
              {message}
            </CAlert>
                    ...
                    <CTable hover responsive>
                      <CTableHead>
                        <CTableRow>
                          ...
                          <CTableHeaderCell className="d-none d-md-table-cell">Dept Status</CTableHeaderCell>
                      ...
                      <CTableBody>
                        {pendingRequests.map((request) => (
                          <CTableRow key={request.id}>
                            ...
                            <CTableHeaderCell className="d-none d-md-table-cell">{request.deptHeadStatus}</CTableHeaderCell>
                            <CTableHeaderCell>
